...
class Warehouse(object):
    ...
    def __str__(self):
        return 'WH(id=%d, %d,%d,products=%s)' % (
            self.id, self.r, self.c,','.join([str(product) for product in self.products]))
...
class Drone(object):
    ...
    def __str__(self):
        return 'Drone(id=%d, (%d,%d), payload=%d)' % (self.id, self.r, self.c,self.payload)
...
class Order(object):
    ...
    def __str__(self):
        return 'Order(id=%d,(%dx%d),items=%s)' % (
            self.id, self.r, self.c,','.join([str(item) for item in self.items]))
...
class Grid(object):
    def __init__(self, rows, cols, num_products, products, num_drones, turns, spayload, warehouses, orders):
        self.rows = rows
        self.cols = cols
        self.grid = []
    ...
    def __str__(self):
        return ('%dx%d (%d drones, %d turns, %d payload):\n'
                ...
                '  drones=%s' % (
                    ...
                    self.payload,
                    str(self.products),', '.join([str(wh) for wh in self.warehouses]),
    ...
    def get_nearest_warehouse_with_product(self, r, c, product_type, num_items,order):
        ...
        for i in range(0, len(self.warehouses)):
            if self.warehouses[i].products[product_type] > 0:
                distance1 = get_distance(r, c,self.warehouses[i].r,self.warehouses[i].c)
                distance2 = get_distance(self.warehouses[i].r,self.warehouses[i].c,order.r,order.c)
    ...
    while True:
        ...
        for drone in grid.drones:
            if drone.busy > 0:
                ...
                if drone.target != (-1, -1):
                    command = (drone.id,'D',drone.target_order_id,drone.target_product_type,drone.target_num_items)
                    ...
                    if _type >= 0:
                        ...
                        if warehouse:
                            ...
                            command = (drone.id,'L', warehouse.id,_type,_count)
                            if drone.load(warehouse, order, _type, _count,turns_left):
                                grid.commands.append(command)
        grid.turn += 1
